...
- **Authored input before use (`useForm`):** declare `definition.useForm` as a `PluginActionFormField[]` (shared with admin plugin actions — includes `number` with optional `min` / `max` / `integer`). The inventory UI opens a form popover; values arrive as `callContext.formValues`. Core validates/coerces in `InventoryService.useItem` before `onItemUsed` ([ADR 0187](adrs/0187-declarative-item-use-forms.md)). When `useForm` is set it takes precedence over `requiresTarget` in the Use button. First consumer: Kickstarter Account (`items/kickstarter-account`). Prefer `useForm` for new multi-field authoring; leave entity pickers on `requiresTarget`.
...
- **Passworded stash container (`storageCapacity`, `requiresTarget: "inventoryItems"` or `"coinAmount"`):** an item with `storageCapacity` *is* a container (ADR 0179). The cubby/case/trailer is consumed into the stash as `containerDefinitionId` and returned when the stash is emptied, or left as an empty stash row to claim later if the bag has no slot for it (ADR 0181). Multi-select sends `targetInventoryItemIds`; optional public `label` (32) / `note` (140) travel with the password. Nesting containers is rejected. Green Room SKUs: Van Cubby (1 / rare / 80), Merch Cash Box (coins / rare / 50), Road Case (3 / legendary / 150), Trailer (5 / legendary / 220). Catalog `coinValue` stays on the rarity ladder (50 / 100); shop markup expresses extra slots.
