...
- **Authored input before use (`useForm`):** declare `definition.useForm` as a `PluginActionFormField[]` (shared with admin plugin actions). The inventory UI opens a form popover; values arrive as `callContext.formValues`. Core validates/coerces in `InventoryService.useItem` before `onItemUsed` ([ADR 0187](adrs/0187-declarative-item-use-forms.md)). Field capabilities:
  ...
  When an entity **`requiresTarget`** and **`useForm`** are both set, the Use button opens the picker first, then the form ([ADR 0193](adrs/0193-item-use-forms-composed-with-target-pickers.md)). Prefer `useForm` for authored values; leave entity pickers on `requiresTarget`. Examples: Kickstarter Account, Burner Phone (`message` + `voice`), Merch Cash Box (`coinAmount` + `password`), Road Case / Van Cubby / Trailer (`inventoryItems` + password/label/note via shared `stashLockFormFields`).
