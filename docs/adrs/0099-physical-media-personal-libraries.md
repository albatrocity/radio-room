...
10. **Collection surfaces the item, not the slots:** The Collection area of the game-state inventory renders only when the caller holds a collection item, without empty slot placeholders (unlike `inventory` — [ADR 0100](0100-dual-inventory-slot-pools.md)). Each Physical Media row deep-links into Add to Queue → Browse → Physical Media with that item selected, via `modalsMachine`'s `EDIT_QUEUE.browseMediaKey`. Durable records have no "use" action, so the deep-link takes that slot.
