...
5. **Client shelves:** `EFFECTIVE_METADATA_SOURCES` (and INIT) include `myMedia: { mediaKey, name, icon?, imageUrl?, artworkFrame? }[]` for the caller’s held playlist-scoped grants. `mediaKey` is the item `shortId`. Add-to-Queue CatalogBrowse gains a **Physical Media** root tab (`RootKind: "media"`).
...
11. **Artwork frames:** Derived Physical Media carries an optional `artworkFrame` presentation token (`"jewel-case" | "record-jacket" | "die-cut-jacket" | "cassette-case"`) on `ItemDefinition`, `ShopOffer`, and `MyMediaShelf`. The value is mapped from the Navidrome playlist prefix (`[CD]` → jewel case, `[LP]` → record jacket, `[45]` → die-cut jacket, `[TAPE]` → cassette case) at derivation time only. It is **never** inferred from operator display-name or icon overrides (`physicalMediaOverrides` has no frame field). The web client renders CSS/SVG overlays when both `imageUrl` and `artworkFrame` are present (`FramedArtwork` with a named size preset: `row` / `track` / `feature` — `feature` prefers `imageUrlLarge`). The layout box is the physical object (square sleeves/jewel cases; portrait 70:110 cassette cases), square corners, drop-shadow. Shop offers, collection inventory, and Add to Queue → Physical Media (shelf list + track list) all go through `toPhysicalMediaArt` into this component. `PluginRegistry.listMyMediaShelves` / `resolveMyMediaShelf` parse frame tokens via `parseArtworkFrame` (the retired `"j-card"` alias maps to `"cassette-case"`).
