# 0081. Live Volume UI via Volume Manager Plugin
...
[ADR 0080](0080-now-playing-seek-and-volume-transport.md) added first-class seek and volume Socket events and a built-in Now Playing transport UI (scrubber + broadcast volume), while removing Volume Manager’s `nowPlayingInfo` slider to avoid duplicate controls.
...
3. **Keep first-class volume Socket/API surface from 0080** — `SET_PLAYBACK_VOLUME`, `volumePercent` / `supportsVolume` on `PLAYBACK_STATE`, and `getPlayback.volumePercent` stay available for plugins, sync, and non-UI callers. Successful volume changes continue to go through `handlePlaybackVolumeChange`.

This **partially supersedes** ADR 0080 decisions (4) and (5) regarding built-in volume UI and hiding the plugin slider. Seek transport and the Socket events from 0080 remain in force.
...
- [0080](0080-now-playing-seek-and-volume-transport.md) — seek + volume Socket events (partially superseded here for UI placement)
