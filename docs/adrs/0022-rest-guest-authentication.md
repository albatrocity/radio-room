...
**Date:** 2026-03-30  
**Status:** Partially superseded by [0058](0058-client-session-localstorage.md)

> **Note:** The **where** the web client persists the guest `userId` (`sessionStorage` under `radio-session-id`) is superseded by [ADR 0058](0058-client-session-localstorage.md) (`localStorage` via `clientSession`). The **REST dual-authorization pattern** in this ADR—`X-Radio-Session-Id`, server `getUser` validation, and resource-scoped binding—remains in force.

## Context

Platform operators and the scheduler app authenticate to the HTTP API with Better Auth ([ADR 0016](0016-better-auth-with-drizzle-for-platform-authentication.md)). The listening room uses a separate identity: a Redis-backed `userId` that the web client persists under `radio-session-id` ([ADR 0058](0058-client-session-localstorage.md): `localStorage` via `clientSession`; ~~formerly `sessionStorage`~~).
...
2. **Guest REST auth:** Otherwise require:
   - HTTP header `X-Radio-Session-Id` whose value is the listening-room Redis user id (same as `radio-session-id` in client storage; see [ADR 0058](0058-client-session-localstorage.md)).
...
- First concrete endpoint: `GET /api/scheduling/shows/:id` — handler `getSchedulingShowByIdHandler`; web client sends `X-Radio-Session-Id` and `roomId` via `fetchShow` in `schedulingApi.ts` (header value from `clientSession.getStoredUserId()` per [ADR 0058](0058-client-session-localstorage.md)).
