...
Track the timestamp when the tab was last confirmed visible. When the tab returns to the foreground:

- **If the absence exceeds a threshold (5 minutes):** send a `FORCE_REFRESH` event to the auth machine, which resets the `initialized` flag and re-emits `LOGIN` with `fetchAllData: true` **while remaining in the `authenticated` state**. The server responds with a full `INIT` payload that every subscribed actor already handles. Staying in `authenticated` is critical: `ChatInput` and other UI components gate visibility/interactivity on `useIsAuthenticated()`, so leaving `authenticated` would hide the chat input and disable interaction during the refresh window.
- **If the absence is below the threshold:** keep the existing incremental sync (`fetchRoom` + `GET_LATEST_ROOM_DATA`).

The `authenticated` state also handles the resulting `INIT` event inline (updating current user and admin status) so the machine never transitions away from `authenticated` during a refresh.
...
- The auth machine stays in `authenticated` throughout the refresh, so `isAuthenticated`-gated UI (chat input, image upload, etc.) remains visible and interactive.
