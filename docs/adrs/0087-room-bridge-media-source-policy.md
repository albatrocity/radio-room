...
`metadataSourceIds` already persisted that policy and drove search fan-out / TrackSearch tabs, but there was no admin UI for YouTube/Library/Tidal toggles while on the bridge controller—only auto-attach on bridge enter ([`withBridgeMetadataSources`](../../packages/server/services/AdminService.ts)).
...
3. **`SET_ROOM_SETTINGS { metadataSourceIds }`** (via Content form submit, or other callers) updates policy while on bridge. Server normalizes via `normalizeBridgeMetadataSourceIds` (require Spotify, drop unknown ids, drop YouTube without `YOUTUBE_API_KEY`). Switching to bridge accepts submitted `metadataSourceIds` when present; otherwise seeds defaults.
