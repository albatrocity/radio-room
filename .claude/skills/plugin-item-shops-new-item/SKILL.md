...
3. **Definition**: **description**, **icon** (Lucide-style name string used by the client, e.g. `chevrons-up`), **rarity** (`common` | `uncommon` | `rare` | `legendary`), **coinValue** (catalog default), **stackable** / **maxStack** / **tradeable** / **consumable**, **requiresTarget** if any (`"self"` | `"user"` | `"queueItem"` | `"inventoryItems"` | `"userInventoryItem"` | `"mediaItem"` | `"storedArtifact"` — see `@repo/types` `ItemDefinition`; `"userInventoryItem"` uses gated `PEEK_USER_INVENTORY`, ADR 0147; `"mediaItem"` is an unfiltered picker over the actor's own stacks for restoration items, ADR 0159; `"storedArtifact"` opens a stash picker and sends `targetArtifactId`, ADR 0185). For authored values (message text, coin amount, password, voice) use **`useForm`** (`PluginActionFormField[]`) — optionally composed with an entity `requiresTarget` (ADR 0187 / 0193). For playback devices set **`slotPool: "playback"`** and **`playbackFormats`** (do not set `mediaFormat` / `artworkFrame`). Set **`gentlePlayback: true`** when the device should skip Physical Media wear-on-queue (ADR 0166). Derived Physical Media uses **`mediaFormat`** + **`slotPool: "collection"`**.
...
| Media Bridge TTS (`useForm` + `optionsSource: "mediaBridgeVoices"`) | Custom `use` + `api.speakOnMediaBridge`; read `callContext.formValues`; consume only on success. See `items/burner-phone` / ADR 0178.                                                                                                                      |
