...
duplicate that pass), `dev-suite:draft-plan` / `dev-suite:review-plan` /
`dev-suite:build-step` (optional follow-through).
...
3. **Plan name** (optional)
   - Default: `plans/review-performance-<kebab-area>.md`
...
- Duplicate `codebase-review` (DRY/sprawl/docs) or a dedicated security review —
...
Save to `plans/review-performance-<kebab-area>.md` (ask before overwrite).
...
- Plan saved as `plans/review-performance-<kebab-area>.md` with opt-outable
  `P#` items and a checklist
