...
Pairs with: `adrs` (constraints), `dev-suite:draft-plan` / `dev-suite:review-plan`
/ `dev-suite:build-step` (optional follow-through on accepted items).

## Ask before running

Do **not** start the audit until the user confirms the review boundary.
Ask once (use `AskUserQuestion`), with a recommended default:
...
3. **Plan name** (optional)
   - Default: `plans/codebase-review-<kebab-area>.md`
...
- Duplicate a dedicated bug-hunt or security review — skip pure bug hunts and
  vuln hunting unless they block a maintainability finding.
...
[apps/web/README.md](apps/web/README.md)). Treat lifted / shared
...
Save to `plans/codebase-review-<kebab-area>.md` (ask before overwrite).
...
2. Apply the user's keep/drop/defer choices in the plan file (`Status:` per
   body item).
3. Only if they ask to implement: hand off to `dev-suite:build-step` one item
   at a time (or implement the kept set if they explicitly request a single
   implementation pass).
...
- Plan saved as `plans/codebase-review-<kebab-area>.md` with opt-outable `R#`
  items and a checklist
