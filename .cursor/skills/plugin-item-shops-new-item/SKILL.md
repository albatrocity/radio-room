...
| Playback device (`slotPool: "playback"`)             | `playbackFormats` on the definition; `usePlaybackDevice` + `playbackDeviceSellbackValue` from `items/shared/playbackDevice.ts`. Do **not** set `mediaFormat` or `artworkFrame`. See ADR 0160.                                                              |
