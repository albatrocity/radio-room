...
Reuse existing patterns (route file + `src/lib/api.ts` + hook + `NavSidebar` entry; auth already gated by [apps/scheduler/src/routes/\_\_root.tsx](apps/scheduler/src/routes/__root.tsx)).
