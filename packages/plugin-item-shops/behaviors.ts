...
async function usePromoteTrack(
  ...
  if (!result.success) {
    const wasBlocked = result.message.startsWith("Blocked by")
    return {
      success: false,
      consumed: wasBlocked,
      message: wasBlocked ? `${result.message}. Your item was still lost.` : result.message,
    }
  }
...
async function useDemoteTrack(
  ...
  if (!result.success) {
    ...
    return {
      success: false,
      consumed: wasBlocked,
      message: wasBlocked ? `${result.message}. Your item was lost with use.` : result.message,
    }
  }
