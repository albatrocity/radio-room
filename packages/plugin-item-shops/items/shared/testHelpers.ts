...
export function createMockPluginAPI(): PluginAPI {
  return {
    ...
    spawnEphemeralUser: vi.fn().mockResolvedValue({ userId: "son-1", username: "Test's son" }),
    despawnEphemeralUser: vi.fn().mockResolvedValue(undefined),
    sendChatMessageAsUser: vi.fn().mockResolvedValue(undefined),
    addReactionAsUser: vi.fn().mockResolvedValue(undefined),
    removeReactionAsUser: vi.fn().mockResolvedValue(undefined),
...
export function createMockDeps(overrides?: Partial<ItemShopsBehaviorDeps>): ItemShopsBehaviorDeps {
  ...
  const resolveDefinition = vi.fn(
    async (definitionId: string, options: { pluginName: string }) => {
      const direct = await getItemDefinition(definitionId)
      if (direct) return direct
      if (!definitionId.includes(":")) {
        return getItemDefinition(`${options.pluginName}:${definitionId}`)
      }
      return null
    },
  )
