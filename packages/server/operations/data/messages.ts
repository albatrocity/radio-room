...
export async function getMessages(roomId: string, offset: number = 0, size: number = 50) {
  try {
...
export async function getMessagesSince(roomId: string, since: number = Date.now()) {
  try {
    ...
    if (!roomExists) {
      ...
      const results = await pubClient.zRangeByScore(roomKey, sinceDate, Date.now())
      return results.map((m) => JSON.parse(m) as ChatMessage) || []
    }
  } catch (e) {
