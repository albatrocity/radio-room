...
import { djDeputizeUser, queueSong, searchForTrack, savePlaylist } from "./djHandlers"
import { addToQueue, getQueue, getUser, isDj, updateUserAttributes } from "../operations/data"
...
describe("djHandlers", () => {
  ...
  describe("queueSong", () => {
    ...
    test("emits SONG_QUEUE_FAILURE event if other user already queued song", async () => {
      ...
      expect(emit).toHaveBeenCalledWith("event", {
        type: "SONG_QUEUE_FAILURE",
        data: {
          message: "Homer has already queued that song. Please try a different song.",
        },
      })
    })
  ...
  describe("searchSpotifyTrack", () => {
    test("calls searchTracks", async () => {
      ...
      await searchForTrack({ socket, io }, { query: "cottoneye joe", options: {} })
  ...
  describe("savePlaylist", () => {
    it("calls createAndPopulateSpotifyPlaylist", async () => {
      ...
      await savePlaylist({ socket, io }, { name: "Hot Jams", uris: ["track1", "track2", "track3"] })
