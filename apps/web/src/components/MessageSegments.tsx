import { Box, Color, Image, Stack } from "@chakra-ui/react"
import type { TextEffect, TextSegment, ColorEffect } from "@repo/types"
import ParsedEmojiMessage from "./ParsedEmojiMessage"
import { textEffectStyles, ColorEffectStyles } from "../lib/textEffects"
import { splitMarkdownImages } from "../lib/splitMarkdownImages"

type InlinePiece = { effects?: TextEffect[]; text: string }
type InlineColor = { effects?: ColorEffect[]; text: string }

type Row = { type: "inline"; items: InlinePiece[] } | { type: "image"; src: string; alt: string }
type RowColor = { type: "inline"; items:InlineColor[] } | { type: "image"; src: string; alt: string }
...
function segmentsToRowsColor(segments: ColorEffect[]): RowColor[] {
  const pieces: Array<{ type: "inline"; piece: InlineColor} | { type: "image"; src: string; alt: string }> = []
...
export function MessageSegmentsColor({ segments }: { segments: ColorEffect[] }) {
  const rows = segmentsToRowsColor(segments)
    ...
    <Stack gap={0.5} align="stretch" w="100%" minW={0}>
      {rows.map((row, i) =>
        row.type === "inline" ? (
          ...
          >
            {row.items.map((item, j) => (
              <Box as="span" key={j} css={ColorEffectStyles(item.effects)} verticalAlign="baseline">
                <ParsedEmojiMessage content={item.text} inlineParagraphs />
              </Box>
            ))}
          </Box>
